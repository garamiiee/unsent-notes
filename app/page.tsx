      ...
      <div className="journey" data-screen={stage === 'compose' ? step : stage}>
        ...
        {stage === 'compose' && <>
          ...
          {step === 2 && <div className={`authentic-editor authentic-${theme}`}>
            {theme === 'chat' ? <>
              <div className="talk-header"><span className="avatar">{p.emoji}</span><strong>{p.name}<small>나만의 가상 채팅방</small></strong><MessageSquare size={20} /></div>
              <div className="talk-conversation"><span className="talk-date">오늘 · 진심을 꺼내는 날</span><p className="talk-guide"><LockKeyhole size={13} />이 대화는 실제 상대에게 전달되지 않습니다.</p><div className="talk-preview">{message || '아래 입력창에 마음을 적어보세요.'}</div><span className="draft-tag">전송 전 미리보기</span></div>
              ...
              <div className="paper-desk"><article className={`a4-paper ${theme}`}>
                ...
                {theme === 'official' ? <><p className="document-end">위와 같이 진심을 전합니다.　끝.</p><div className="official-sign">마 음 정 리 사 무 소 장 <span className="seal">가상<br />직인</span></div><div className="official-bottom">시행　마음-001　　접수　가상 전달 후 접수<br />담당　오늘의 나　　공개 구분　나만 보기</div></> : <><p className="document-end">2. 기대 효과: 조금 더 가벼운 마음으로 하루를 마무리함.</p><div className="paper-page">- 1 -</div></>}
              </article></div><div className="document-status"><span>1 / 1쪽　·　{message.length} / 2,000자</span><span>가상 문서 · 실제 제출 안 됨</span></div>
            </>}
          </div>}
